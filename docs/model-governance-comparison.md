...
以下能力暂时关闭入口，当前迭代不继续投入，优先级为 **P3 / 低**：

| 后续项 | 当前处理 | 暂缓原因 | 恢复前置条件 |
|--------|----------|----------|--------------|
| **平台详情页** | 平台卡片不再响应点击，详情实现暂时保留 | 详情依赖的模型、端点、认证和使用数据目前无法稳定同步，展示容易造成误导 | 先建立 provider → offering → endpoint → model availability 的完整同步与一致性校验，再重新评估详情的信息架构 |
| **模型对比页** | 隐藏“模型对比”入口，页面实现暂时保留 | 当前没有明确决策场景，对比结果也缺少统一、可信的数据来源 | 明确用户需要据此完成的选择任务，并补齐模型能力、价格、上下文与可用端点的数据质量后再恢复 |

恢复以上入口时，需要同时补充数据来源说明、空数据状态、同步失败状态和对应的端到端测试，不能仅重新显示 UI。
