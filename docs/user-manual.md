...
- **云同步**：端到端加密的跨机器同步，支持 iCloud / WebDAV / Cloudflare / Supabase / 火山引擎 KMS 等平台。在设置中配置同步密码与平台后：
  ```bash
  okit vault push     # 推送密钥到云端
  okit vault pull     # 从云端拉取
  ```
  云端只存密文，主密钥始终在本机。
- **自动同步**：在设置 → 跨机器同步中开启「自动同步」后，无需手动推送/拉取：
  ...
  - 首次开启会立即推送一次建立云端基线；多台机器用同一同步密码即可互相同步
- **局域网同步**：同一局域网内的电脑之间直接同步，不需要任何云平台账号（设置 → 设备同步 → 添加设备）：
